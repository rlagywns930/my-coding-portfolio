# 🛳 Titanic 생존자 분석 - EDA


---

## 📋 목차

1. [데이터 불러오기](#1-데이터-불러오기)  
2. [데이터 구조 및 요약](#2-데이터-구조-및-요약)  
3. [결측치 확인](#3-결측치-확인)
4. [Target label 확인](#4-target-label-확인)
5. [변수별 생존률 분석](#5-변수별-생존률-분석)  
   - [Pclass (객실 등급)](#pclass-객실-등급)  
   - [Sex (성별)](#sex-성별)
   - [Pclass (객실 등급), Sex (성별) 비교](#pclass-객실-등급-sex-성별-비교)  
   - [Age (나이)](#age-나이)
   - [Age, Sex, Pclass에 따른 비교](#age-sex-pclass에-따른-비교)
   - [Embarked (탑승 항구)](#embarked-탑승-항구)
   - [SibSp / Parch (가족 수)](#sibsp--parch-가족-수)  
   - [Fare (운임 요금)](#fare-운임-요금)
   - [Ticket (티켓 번호)](#ticket-티켓-번호)
  

---
# 1. 데이터 불러오기

![image](https://github.com/user-attachments/assets/2f872627-97a8-4470-8a08-1ec8bdedc9d8)

![image](https://github.com/user-attachments/assets/07bb170c-e439-4708-bdeb-2569f985e8e3)

---
# 2. 데이터 구조 및 요약

![image](https://github.com/user-attachments/assets/5f0b5a1b-6f47-4fee-bfd5-621c575e9011)

![image](https://github.com/user-attachments/assets/abec454a-ff6d-4c79-bb67-e62e756eedeb)
---

![image](https://github.com/user-attachments/assets/8696a9a9-bde0-4b22-95ba-adbdad0ea737)

![image](https://github.com/user-attachments/assets/878087dd-5933-44c2-8bb4-a02a68d0921b)
---

![image](https://github.com/user-attachments/assets/08507a9d-d29d-4a60-979a-87151d14e575)

![image](https://github.com/user-attachments/assets/81cfc3f3-200b-4afe-9045-8006b4e39842)
---

![image](https://github.com/user-attachments/assets/b6d84703-8002-43aa-8ac5-001b1876631d)

![image](https://github.com/user-attachments/assets/36e324b2-1819-4abb-aff8-081d83cfcd80)  
---

## 🔍 Train vs Test 데이터셋 차이점

| 항목        | train.csv (df_train) | test.csv (df_test) |
|-------------|----------------------|---------------------|
| 샘플 수      | 891                  | 418                 |
| Survived 컬럼 | ✅ 있음              | ❌ 없음            |
| 목적        | 모델 학습용           | 예측 및 제출용       |
| 공통 피처     | Pclass, Sex, Age, SibSp, Parch, Fare, Cabin, Embarked 등 

### ✅ 주요 차이 요약

- `train.csv`는 `Survived` 값이 포함되어 있어 모델이 학습할 수 있다.
- `test.csv`는 `Survived`가 없고, 예측 결과를 기반으로 제출하는 용도다.
- 동일한 전처리를 적용하되, 결측치 처리 등은 따로 확인해주어야 한다.

---
# 3. 결측치 확인

![image](https://github.com/user-attachments/assets/24b5af2f-06e4-424d-a159-570d7242b13e)

![image](https://github.com/user-attachments/assets/bf9fb7df-5bb4-4827-8b7f-ab6cfd959772)
---

![image](https://github.com/user-attachments/assets/90fa72bc-5032-44c1-9097-902ddcf79e2f)

![image](https://github.com/user-attachments/assets/714e9ffa-408a-48ef-837e-61a976a0866d)
---

![image](https://github.com/user-attachments/assets/384da982-97ce-44e7-bd00-adda46671a74)

![image](https://github.com/user-attachments/assets/8bd5aca5-9ecd-4cfa-b00e-14d0ffc01a4d)
---

![image](https://github.com/user-attachments/assets/5443a959-d629-4e83-8d7a-d701cdbbb851)

![image](https://github.com/user-attachments/assets/191c2b7f-902f-4ef6-8737-2ddaa92c8e25)

---
# 4. Target label 확인

![image](https://github.com/user-attachments/assets/1e7e54ad-259c-4b75-b234-30cdc6e77ca0)

![image](https://github.com/user-attachments/assets/4d46e081-84bb-4644-b2c2-72e03266e65e)  

그래프를 통해 Titanic 전체 승객 중 38.4%만 생존 헀다는 것을 알 수 있다 즉 생존자보다 사망자가 훨씬 많다는 것을 알 수 있다
전체 891명 중 61.6% 사망 (0), 38.4% 생존 (1) 이 지표에서 보이듯 뷸균형한 데이터를 통해 편향된 예측을 할 가능성을 보임


---
# 5.1 pclass (객실 등급)

![image](https://github.com/user-attachments/assets/c308a1f1-389c-4079-a93c-20345bd6dd98)

![image](https://github.com/user-attachments/assets/e912bbeb-1925-41da-bcf7-686665ebd854)  

1등석: 216명 중 136명 생존 → 생존률 약 63%

2등석: 184명 중 87명 생존 → 생존률 약 47%

3등석: 491명 중 119명 생존 → 생존률 약 24%

등급이 높을 수록 생존 확률이 높다는 것을 명확히 보여줌

---

![image](https://github.com/user-attachments/assets/11ac7cd1-996e-4449-96fd-e07ef29d2c80)

![image](https://github.com/user-attachments/assets/1ea247a0-8f55-4fc0-8440-7676e4b34934)  

평균값을 통해 각 등급의 생존률만 추출

위와 동일하게 1등석 > 2등석 > 3등석 순서로 생존률이 감소

---


![image](https://github.com/user-attachments/assets/fd62735c-55f5-45d2-a4a3-3c90e7e43aa7)

![image](https://github.com/user-attachments/assets/1cded569-8277-4ad9-ae2f-618de7665791)

[왼쪽 그래프]
각 등급별 탑승자 수 비교

→ 3등석 탑승자가 가장 많고, 1등석이 가장 적음

[오른쪽 그래프]
각 등급별로 생존자(1) vs 사망자(0) 수 비교

시각적으로도 1등석은 생존자가 많고,
3등석은 사망자가 압도적으로 많음을 보여줌

[결론: 이 분석은 객실 등급이 생존율에 중요한 영향을 미친다는 것을 수치와 시각화를 통해 명확히 보여주며, Pclass가 모델링에 있어 중요한 변수라는 인사이트를 제공함]

---
# 5.2 Sex (성별)

![image](https://github.com/user-attachments/assets/3800278a-d4c5-4a50-82ae-2aed9f2f91a3)

![image](https://github.com/user-attachments/assets/674e97cd-f579-441e-94f5-3a73064507f6)  

여성 314명 중 233명 생존 → 생존률 약 74%
남성 557명 중 109명 생존 → 생존률 약 19%
=> 여성 생존률이 훨씬 높음

---

![image](https://github.com/user-attachments/assets/355db28a-09f6-48b6-ac87-4ae7ec1cad5f)

![image](https://github.com/user-attachments/assets/13da21d2-1d94-41a9-a742-24e45762a714)

수치로 봐도 여성 생존률이 4배 이상 높음
 
---

![image](https://github.com/user-attachments/assets/742c2cc9-ab79-474d-a12b-b34e8f8fc3fd)

![image](https://github.com/user-attachments/assets/daf72e7d-0dad-46fd-bc15-8b884ce8c28a)

[왼쪽 그래프]
여성의 생존률(0.74)이 시각적으로 높게 표시됨

남성은 약 0.19로 낮음

[오른쪽 그래프]
여성은 생존자가 더 많고

남성은 사망자가 훨씬 많음

시각적으로도 성별에 따른 극단적인 차이 확인 가능

[결론: 이 분석은 Titanic에서 성별이 생존 확률에 강한 영향을 미쳤다는 것을 수치와 시각화를 통해 명확히 보여주며, 특히 여성은 생존률이 74%에 달한 반면, 남성은 19%에 불과했다는 극단적인 차이를 확인할 수 있다.]

---

# 5.3 Pclass (객실 등급), Sex (성별) 비교

![image](https://github.com/user-attachments/assets/626659a5-1e33-479b-ae77-fd6451081864)

![image](https://github.com/user-attachments/assets/9c38b7ec-cd01-4465-a384-9af69204d238)
---

![image](https://github.com/user-attachments/assets/26531fe4-7e86-4988-8a8a-db1f173964b2)

![image](https://github.com/user-attachments/assets/1b2fe5f7-3ec2-42d7-8c43-6e5a2b659a55)
---

- 모든 등급에서 여성의 생존률이 남성보다 높음
- 1등석 여성 생존률이 가장 높고, 3등석 남성 생존률이 가장 낮음
- 성별과 등급은 **생존률에 복합적으로 영향을 미치는 주요 변수**
- 성별이 등급보다 더 강한 영향을 미치는 경우도 존재함

---

# 5.4 Age (나이)

![image](https://github.com/user-attachments/assets/5c9b8088-03ee-4164-b896-4e780d7d3983)

![image](https://github.com/user-attachments/assets/f3549dd1-28c2-45ba-9717-285c35ef13a6)
---

![image](https://github.com/user-attachments/assets/2ce4887c-5a97-47af-b3d7-68809ccd98b3)

![image](https://github.com/user-attachments/assets/6c3dad18-e681-4cb4-8034-65c557fb6b2d)

- 생존자 중 **0~10세 어린이 비율**이 상대적으로 높음
- 사망자는 **20~40세 성인 남성** 비율이 두드러짐
- 구조 우선순위가 **어린이와 여성**에게 있었음을 암시

---

![image](https://github.com/user-attachments/assets/744c5045-335b-4fe6-a92a-fe475fd46f78)

![image](https://github.com/user-attachments/assets/610e2ded-7fdc-4566-8bec-f64fc568aa03)

- 1등석: **고령자 중심**, 다양한 연령대
- 2등석: **20~40대 성인 중심**
- 3등석: **어린 승객 비율 높음**, 가족 단위 탑승 예상

[결론: 생존자 분석에서 나이는 중요한 변수이며, 특히 어린이일수록 생존 확률이 높은 경향이 있슴. 또한 객실 등급(Pclass) 과 나이 분포는 서로 밀접한 관계가 있으며, 이는 탑승자의 사회·경제적 배경이 생존에 영향을 줄 수 있다는 중요한 인사이트를 제공함]

---
# 5.5 Age (나이),Sex (성별), Pclass (객실 등급)에 따른 비교

![image](https://github.com/user-attachments/assets/13efdb4d-beb7-4418-84bd-bd9c1e1333ce)

![image](https://github.com/user-attachments/assets/461bf268-8bd9-4c50-b7f1-7b12573eda9b)

[Pclass and Age vs Survived]
- **1등석 승객**은 고령자 포함 다양한 연령대에서 **생존률 높음**
- **2등석**은 중간 연령층 중심, 생존률 중간
- **3등석**은 어린 승객 존재하지만, **전반적인 사망률 매우 높음**

[Sex and Age vs Survived]
- **여성은 모든 연령대에서 생존률이 높음**
- **남성은 특히 20~40세에서 사망률 집중**
- **어린이 생존률 높음** (남녀 공통)

[결론: 이 두 그래프는 Titanic 구조 과정에서 '여성과 아이 우선'이라는 규칙이 실제로 데이터에 반영되었음을 잘 보여줌.]

---

# 5.6 Embarked (탑승 항구)

![image](https://github.com/user-attachments/assets/5e8e6080-3075-42af-aeb4-6ce97f476d1a)

![image](https://github.com/user-attachments/assets/3dcfd91e-be3e-47a7-aa50-e61a77c84f05)

- **Cherbourg(C)**: 생존률이 가장 높음
- **Queenstown(Q)**: 중간
- **Southampton(S)**: 생존률이 가장 낮음

---

![image](https://github.com/user-attachments/assets/c45419ac-7091-4be0-ade9-fb851fd1ba64)

![image](https://github.com/user-attachments/assets/bf9f4a3e-a074-4104-98f7-44cbf853fe3b)

| Plot | 설명 |
|------|------|
| (1) No. Of Passengers Boarded | S(Southampton)에서 가장 많은 승객이 탑승 |
| (2) Male-Female Split for Embarked | S 항구는 남성 비중이 높고, C는 여성 비중이 상대적으로 높음 |
| (3) Embarked vs Survived | C 탑승자 생존률 높음, S는 낮음 |
| (4) Embarked vs Pclass | C는 1등석 승객 많고, S/Q는 3등석 많음 |

[결론: 탑승 항구는 생존률과 직접적인 연관은 없지만, **성별과 객실 등급이라는 주요 변수와 밀접하게 연결되어** 있어 생존률에 간접적으로 큰 영향을 주는 피처임]

---

# 5.7 SibSp / Parch (가족 수)

![image](https://github.com/user-attachments/assets/7ef77a07-877d-4232-91bb-18b649292bbb)
---

![image](https://github.com/user-attachments/assets/400e1253-d309-4815-95ac-2957690c2f3b)

![image](https://github.com/user-attachments/assets/50288ab1-9cc2-44a3-8aa2-82501b0e30e9)
---

![image"](https://github.com/user-attachments/assets/81584039-e02d-479f-8653-060ea6cbc858)

![image](https://github.com/user-attachments/assets/b8e68d26-10a5-409c-bac8-60fa1ac2e24d)


| 그래프 | 설명 |
|--------|------|
| (1) 전체 FamilySize 분포 | 대부분 1~4명으로 구성 |
| (2) FamilySize별 생존/사망자 수 | 소가족(2~4명) 생존률 높음 |
| (3) FamilySize별 평균 생존률 | 1명 혹은 5명 이상일 때 생존률 급감 |

[결론: 가족 규모가 너무 작거나 너무 크면 생존 확률이 낮아지며, **소규모 가족이 가장 생존에 유리**함.]

---

# 5.8 Fare (운임 요금)

![image](https://github.com/user-attachments/assets/cbf770d3-1e3a-4589-9c85-34b7e6c2c8be)

**결측치 처리 (Missing Value Imputation)**
   - `df_test`에 있는 `Fare`의 결측값을 평균값으로 대체

**로그 변환 (Log Transformation)**
   - `Fare` 값의 왜도(skewness)가 높아 정규 분포에 가깝게 만들기 위해 로그 변환 적용
---

![image](https://github.com/user-attachments/assets/b151110a-fc3f-4ffd-85bb-10cacf5f30c9)

![image](https://github.com/user-attachments/assets/a1d921c8-f696-498c-b0a8-0af8e81a6ba8)

1. **대부분의 승객은 저렴한 요금을 지불했다**
   - 요금이 낮은 구간에 데이터가 몰려 있으며, 이는 주로 3등석 승객들과 관련이 있음

2. **일부 상류층 승객은 매우 높은 요금을 지불했다**
   - 로그 변환 전에는 오른쪽 꼬리가 길었으며, 이는 소수의 상류층 승객들이 고가의 티켓을 구매했음을 의미

3. **`Fare`는 생존 가능성과 관련된 중요한 변수이다**
   - 일반적으로 요금이 높을수록 상류층일 가능성이 크며, 이들은 구조 우선순위가 높아 생존률도 높았음

4. **로그 변환을 통해 데이터 분포가 정규화되어 모델 학습이 더 안정적으로 이루어짐**
   - 왜도(Skewness)가 줄어들면서, 머신러닝 모델이 `Fare` 값을 더 효과적으로 학습할 수 있는 환경이 조성됨  
  
[결론: Fare는 단순한 숫자가 아닌, 승객의 사회적 지위와 생존 가능성을 반영하는 중요한 지표이다]

# 5.9 Tiket(티켓 번호)

![image](https://github.com/user-attachments/assets/d3bec5c7-5bd2-47b3-b72c-c5a0eddbd78a)

![image](https://github.com/user-attachments/assets/fe8b0e6a-32ba-4b89-9e8c-22c905a0ecbe)

1. 같은 티켓 번호를 가진 승객들이 존재
- 동일한 Ticket 번호가 여러 번 등장하는 것으로 보아, 함께 탑승한 가족, 친구, 그룹 단위의 승객들이 있었음을 알 수 있음

2. 동행 여부나 그룹 규모를 추정할 수 있음
- 같은 티켓을 공유한 승객 수를 통해 동행인 수나 혼자 탑승했는지 여부를 추정할 수 있음

3. 모델 학습에 활용 가능한 추가 피처 생성 가능
- Ticket 번호의 중복 횟수를 새로운 변수로 추가하여, 생존 예측에 도움이 되는 특징을 반영할 수 있음

[결론: Ticket 컬럼은 단순한 문자열 같지만, 탑승 그룹의 존재 여부를 파악할 수 있는 중요한 정보이다.]





